...
const BROKEN_STORY_ASSET_IDS = new Set(
  Object.values(storyData).flatMap((story) => [
    ...
    ...story.sections
      .filter(
        (section): section is StorySection & { publicId: string } =>
          section.type === 'image' && Boolean(section.publicId),
      )
      .map((section) => section.publicId),
  ]),
);

function getStoryPalette(publicId?: string) {
